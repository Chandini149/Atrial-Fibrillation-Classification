...
A closer look at class-level ROC-AUC scores adds further nuance to these results. The "O" (Other) rhythm class showed the lowest ROC-AUC of all four classes in every model (0.67 for Linear SVM, 0.76 for LightGBM and the hybrid model), lower even than the noisy class. This pattern was consistent with the confusion matrices, where roughly half of all true "O" segments were misclassified as "N", and with the bivariate feature distributions explored during EDA, where "O" visually overlapped with "N" across several engineered features, including poincare_sd1, sdnn, and std_dev. This suggests that the engineered features used in this analysis do not clearly separate "Other" rhythm from "Normal" rhythm, and that this overlap was the main driver of overall classification error.

By contrast, the noisy class showed a notably higher ROC-AUC (0.86–0.91) than its precision and recall would suggest. This reflects the difference between ranking-based and threshold-based evaluation: ROC-AUC measures how well a model ranks true positives above true negatives across all possible thresholds, while precision and recall reflect performance at a single fixed decision threshold. With only 188 noisy segments in the test set, models were able to rank most noisy segments above non-noisy segments, which increased AUC, while still rarely selecting "noisy" as the single most likely class for any individual segment, which kept precision and recall low.
